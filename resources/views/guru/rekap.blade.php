...
<div class="container py-5">
    ...
    <h2 class="mb-5 text-center fw-bold text-orange-gradient">
        <i class="bi bi-journal-check"></i> Rekap Absen Guru {{ $kelas ?? 'Semua Mapel' }}
    </h2>
            ...
            <div class="card-body d-flex align-items-center">
                ...
                <div class="ms-3">
                    <h5 class="fw-bold mb-2 text-dark">
                        {{ $absen->guru->nama ?? 'Guru' }}
                    </h5>
                    <p class="mb-1 text-muted">
                        <strong>Mata Pelajaran:</strong> {{ $absen->guru->mapel ?? 'Belum Diisi' }}
                    </p>
                    <p class="mb-2 text-muted">
                        <strong>Tanggal:</strong> {{ \Carbon\Carbon::parse($absen->tanggal)->format('d M Y') }}
                    </p>
                    <span class="badge badge-elegant">
                        {{ $absen->keterangan }}
                    </span>
                </div>
            </div>
    ...
    @empty
        <div class="alert alert-warning text-center shadow-lg rounded-3 fw-bold">
            Belum ada data absen guru untuk <span class="text-orange-gradient">{{ $kelas ?? 'semua kelas' }}</span>.
        </div>
    @endforelse
</div>
